  ...
  try {
    ...
    const newTable = await prisma.storeTable.create({
      data: {
        ...
        table_number,
        qr_code: '', // nanti diupdate setelah QR-nya diupload
      },
    })

    // 2️⃣ Generate URL yang akan disematkan di QR code
    const qrTargetUrl = `${process.env.NEXT_PUBLIC_BASE_URL}/menu/${newTable.id}`
    ...
    const fileName = `table-${newTable.id}-${Date.now()}.png`
