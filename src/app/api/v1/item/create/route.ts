...
export async function POST(req: Request) {
  try {
    ...
    if (isNaN(Number(price))) {
      return NextResponse.json({ message: 'Price harus berupa angka yang valid' }, { status: 400 })
    }
    ...
    if (uploadError) {
      ...
      return NextResponse.json({ message: 'Gagal upload ke Supabase', error: uploadError.message }, { status: 500 })
    }

    //  Ambil public URL dari file yang diupload
    const { data: publicUrlData } = supabase.storage.from(process.env.SUPABASE_BUCKET!).getPublicUrl(uploadData.path)
